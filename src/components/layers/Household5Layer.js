...
export const HOUSEHOLD5_LAYER_ID = 'household5Layer';
...
export default function Household5Layer() {
  const dispatch = useDispatch();
  const { household5Layer } = useSelector((state) => state.carto.layers);
  const source = useSelector((state) => selectSourceById(state, household5Layer?.source));
  const cartoLayerProps = useCartoLayerProps({ source });

  if (household5Layer && source) {
    return new CartoLayer({
      ...
      getFillColor: d => {
        ...
        if(d.properties.amount_of_collected_garbage_in_ton_2020 <= 100000){
          return color1; 
        }else if(d.properties.amount_of_collected_garbage_in_ton_2020 <= 500000){
          return color2;
        }else if(d.properties.amount_of_collected_garbage_in_ton_2020 <= 1000000){
          return color3;
        }else if(d.properties.amount_of_collected_garbage_in_ton_2020 <= 5000000){
          return color4;  
        }else{
      ...
      onClick: (info, event) =>{ 
        ...
        if(document.getElementById('custom-tooltip')){
         ...
         elem.parentNode.removeChild(elem);
          ...
          .replaceAll('amount_of_collected_garbage_in_ton_2020','Amount of Collected Garbage in Ton (2020)')
