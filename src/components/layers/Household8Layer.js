...
export const HOUSEHOLD8_LAYER_ID = 'household8Layer';
...
const layerConfig = {
  title: 'Dropout in Primary Education',
  visible: true,
  switchable: false,
  legend: {
    attr: 'Percent',
...
export default function Household8Layer() {
  const dispatch = useDispatch();
  const { household8Layer } = useSelector((state) => state.carto.layers);
  const source = useSelector((state) => selectSourceById(state, household8Layer?.source));
  const cartoLayerProps = useCartoLayerProps({ source });

  if (household8Layer && source) {
    return new CartoLayer({
      ...
      getFillColor: d => {
        ...
        if(d.properties.dropout_percentage_in_primary_education_2017_2018_2018_2019 <= 0.1){
          return color1; 
        }else if(d.properties.dropout_percentage_in_primary_education_2017_2018_2018_2019 <= 0.3){
          return color2;
        }else if(d.properties.dropout_percentage_in_primary_education_2017_2018_2018_2019 <= 0.5){
          return color3;
        }else if(d.properties.dropout_percentage_in_primary_education_2017_2018_2018_2019 <= 0.7){
          return color4;  
        }else{
      ...
      onClick: (info, event) =>{ 
        ...
        if(document.getElementById('custom-tooltip')){
         ...
         elem.parentNode.removeChild(elem);
          ...
          .replaceAll('dropout_percentage_in_primary_education_2017_2018_2018_2019','Dropout Percentage in Primary Education')
          .replaceAll('<strong>governorate</strong>','<strong>Governorate</strong>')
          .replaceAll('<strong>male_dropout_percentage_in_primary_education_2017_2018_2018_201</strong>','<strong>Male Dropout Percentage in Primary Education</strong>')
          .replaceAll('<strong>female_dropout_percentage_in_primary_education_2017_2018_2018_2</strong>','<strong>Female Dropout Percentage in Primary Education</strong>')
