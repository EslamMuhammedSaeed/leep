...
export const HOUSEHOLD13_LAYER_ID = 'household13Layer';
...
export default function Household13Layer() {
  const dispatch = useDispatch();
  const { household13Layer } = useSelector((state) => state.carto.layers);
  const source = useSelector((state) => selectSourceById(state, household13Layer?.source));
  const cartoLayerProps = useCartoLayerProps({ source });

  if (household13Layer && source) {
    return new CartoLayer({
      ...
      getFillColor: d => {
        ...
        if(d.properties.average_number_of_hospitals_100k_of_residents <= 1){
          return color1; 
        }else if(d.properties.average_number_of_hospitals_100k_of_residents <= 3){
          return color2;
        }else if(d.properties.average_number_of_hospitals_100k_of_residents <= 5){
          return color3;
        }else if(d.properties.average_number_of_hospitals_100k_of_residents <= 7){
          return color4;  
        }else{
      ...
      onClick: (info, event) =>{ 
        ...
        if(document.getElementById('custom-tooltip')){
         ...
         elem.parentNode.removeChild(elem);
          ...
          .replaceAll('average_number_of_hospitals_100k_of_residents','Average Number of Hospitals / 100K Residents')
