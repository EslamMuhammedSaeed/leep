...
export const HOUSEHOLD3_LAYER_ID = 'household3Layer';
const color1 = [254,237,222];
const color2 = [253,190,133];
const color3 = [253,141,60];
const color4 = [230,85,13];
const color5 = [166,54,3];
...
export default function Household3Layer() {
  const dispatch = useDispatch();
  const { household3Layer } = useSelector((state) => state.carto.layers);
  const source = useSelector((state) => selectSourceById(state, household3Layer?.source));
  const cartoLayerProps = useCartoLayerProps({ source });

  if (household3Layer && source) {
    return new CartoLayer({
      ...
      getFillColor: d => {
        ...
        if(d.properties.percentage_of_households_connected_to_the_public_network_of_sew <= 20){
          return color1; 
        }else if(d.properties.percentage_of_households_connected_to_the_public_network_of_sew <= 50){
          return color2;
        }else if(d.properties.percentage_of_households_connected_to_the_public_network_of_sew <= 70){
          return color3;
        }else if(d.properties.percentage_of_households_connected_to_the_public_network_of_sew <= 85){
          return color4;  
        }else{
      ...
      onClick: (info, event) =>{ 
        ...
        if(document.getElementById('custom-tooltip2')){
         ...
         elem2.parentNode.removeChild(elem2);
          ...
          .replaceAll('percentage_of_households_connected_to_the_public_network_of_sew','Percentage of Households Connected')
