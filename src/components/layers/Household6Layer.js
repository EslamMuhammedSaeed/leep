...
export const HOUSEHOLD6_LAYER_ID = 'household6Layer';
const color1 = [255,255,178];
const color2 = [254,204,92];
const color3 = [253,141,60];
const color4 = [240,59,32];
const color5 = [189,0,38];
...
export default function Household6Layer() {
  const dispatch = useDispatch();
  const { household6Layer } = useSelector((state) => state.carto.layers);
  const source = useSelector((state) => selectSourceById(state, household6Layer?.source));
  const cartoLayerProps = useCartoLayerProps({ source });

  if (household6Layer && source) {
    return new CartoLayer({
      ...
      getFillColor: d => {
        ...
        if(d.properties.amount_of_collected_municipal_waste_in_ton_2019 <= 50){
          return color1; 
        }else if(d.properties.amount_of_collected_municipal_waste_in_ton_2019 <= 100){
          return color2;
        }else if(d.properties.amount_of_collected_municipal_waste_in_ton_2019 <= 1000){
          return color3;
        }else if(d.properties.amount_of_collected_municipal_waste_in_ton_2019 <= 5000){
          return color4;  
        }else{
      ...
      onClick: (info, event) =>{ 
        ...
        if(document.getElementById('custom-tooltip')){
         ...
         elem.parentNode.removeChild(elem);
          ...
          .replaceAll('amount_of_collected_municipal_waste_in_ton_2019','Amount of Collected Municipal Waste in Ton (2019)')
