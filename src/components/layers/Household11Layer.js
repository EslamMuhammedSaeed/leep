...
export const HOUSEHOLD11_LAYER_ID = 'household11Layer';
const color1 = [237,248,233];
const color2 = [186,228,179];
const color3 = [116,196,118];
const color4 = [49,163,84];
const color5 = [0,109,44];
...
export default function Household11Layer() {
  const dispatch = useDispatch();
  const { household11Layer } = useSelector((state) => state.carto.layers);
  const source = useSelector((state) => selectSourceById(state, household11Layer?.source));
  const cartoLayerProps = useCartoLayerProps({ source });

  if (household11Layer && source) {
    return new CartoLayer({
      ...
      getFillColor: d => {
        ...
        if(d.properties.total_gdp_in_thousand_egp_2015_2016 <= 10000000){
          return color1; 
        }else if(d.properties.total_gdp_in_thousand_egp_2015_2016 <= 100000000){
          return color2;
        }else if(d.properties.total_gdp_in_thousand_egp_2015_2016 <= 500000000){
          return color3;
        }else if(d.properties.total_gdp_in_thousand_egp_2015_2016 <= 900000000){
          return color4;  
        }else{
